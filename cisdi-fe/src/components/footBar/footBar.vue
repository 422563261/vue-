<template>
  <div class="foot-bar">
    <ul class="bar-all" v-if="moreSeven">
      ...
      <li class="bar-list" :class="{'active': this.currentPg === 2}" @click="clickNormal($event)">
        {{five - 3 | igFirst}}
      </li>
      <li class="bar-list" :class="{'active': this.currentPg === five - 2}" @click="clickThree()">{{five - 2}}</li>
      <li class="bar-list" :class="{'active': this.currentPg === five - 1}" @click="clickNormal($event)">{{five - 1}}
      </li>
      <li class="bar-list" :class="{'active': this.currentPg === five}" @click="clickFive()">{{five}}</li>
      <li class="bar-list" :class="{'active': this.currentPg === pgAll - 1}" @click="clickNormal($event)">
        {{pgAll - 1 | igSecond(five)}}
      </li>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      jump(page) {
        let {pgAll, pageArr, currentPg} = format(this.rn, page);
        this.currentPg = parseInt(currentPg);
        this.$emit('changeCD', pageArr);
        if (this.currentPg === 1) {
          this.five = 5;
